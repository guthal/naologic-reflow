...
function wo(id: string, dependsOnWorkOrderIds: string[] = []): WorkOrderDoc {
  return {
    docId: id,
    docType: "workOrder",
    data: {
...
test("dag creation fails if a dependency is missing", () => {
  assert.throws(
    () => buildWorkOrderDependencyDag([wo("a", ["missing-id"])]),
    /Missing dependency missing-id referenced by a/,
  );
});
