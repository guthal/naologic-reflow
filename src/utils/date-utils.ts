...
import type { WorkCenterDoc, WorkOrderDoc } from "../reflow/types.js";
...
export function toUtc(value: string): DateTime {
...
export function toIsoUtc(value: DateTime): string {
  return value.toUTC().toISO({ suppressMilliseconds: true }) ?? value.toUTC().toISO()!;
}

export function buildBlockedWindows(
  workCenter: WorkCenterDoc,
  fixedMaintenanceOrders: WorkOrderDoc[],
): TimeWindow[] {
...
function inShift(workCenter: WorkCenterDoc, at: DateTime): boolean {
...
function getShiftEnd(workCenter: WorkCenterDoc, at: DateTime): DateTime | null {
...
function nextShiftStart(workCenter: WorkCenterDoc, after: DateTime): DateTime {
  ...
  throw new Error(
    `No shift availability found for work center ${workCenter.docId}. Check shift config.`,
  );
}
...
function getNextBlockStart(blockedWindows: TimeWindow[], after: DateTime): DateTime | null {
  for (const window of blockedWindows) {
...
export function findEarliestWorkingMoment(
  workCenter: WorkCenterDoc,
...
export function calculateEndDateWithCalendar(
  ...
  while (remaining > 0) {
    ...
    const availableUntil =
      nextBlockStart && nextBlockStart < shiftEnd ? nextBlockStart : shiftEnd;
    ...
    const availableMinutes = Math.floor(
      Interval.fromDateTimes(cursor, availableUntil).toDuration("minutes").minutes,
    );
...
export function minutesDiff(a: DateTime, b: DateTime): number {
